...
$(function(){
    ...
    var width = $('body').width(),
        fontsize_20 = 20, fontsize_16 = 16, fontsize_14 = 14, grid_left = 75, grid_right = 70, grid_top = 60, grid_bottom = 90, itemsize = 10, itemgap = 10, barmaxwidth = 50, symbolSize = 8, marksymbol = [110,40], symboloffset = [0,30];
    ...
    var uid = '5',
        ...
        baseUrl = 'http://192.168.4.50/FSO.Net/FSO/TelevisionService.svc/';
    ...
    var func = {
        ...
        setMaindata : function(json){
            var _self = this;
            $('#year_mission').html(COMM.formatNumber(json.data.year_mission,0));
            $('#year_over').html(COMM.formatNumber(json.data.year_finish,0));
            ...
            $('#day_total').html(COMM.formatNumber(json.data.day_total,0));
            $('#week_total').html(COMM.formatNumber(json.data.week_total,0));
            $('#month_total').html(COMM.formatNumber(json.data.month_total,0));
        },
        setCharts : function(json){
            if(json.xdata.length > 0){
                ...
                for(var i = 0;i < len;i++){
                    ...
                    }else if(json.series[0].rate_data[i] > 0 && json.series[0].rate_data[i] <= 50){
                        data_res.push({
                            value: json.series[0].rate_data[i],
                            itemStyle:{
                                normal:{
                    ...
                    }else if(json.series[0].rate_data[i] <= 0){
                        data_res.push({
                            value: json.series[0].rate_data[i],
                            itemStyle:{
                                normal:{
        ...
        teamRank : function(json){
            ...
            $.each(json.data,function(idx,val){
                var this_data = this, val = COMM.formatNumber(this_data.issue,0);
                if(idx <= 2){
                    htm += '<tr>'+
        ...
        groupRank : function(json){
            var group1_left = '', group1_right = '', group2_left = '', group2_right = '',
                _self = this;
            $.each(json.data,function(idx,val){
                var this_data = this, val = COMM.formatNumber(this_data.issue,0);
                if(idx < 7){
            ...
            group2_right += '<tr><td></td><td></td><td></td></tr><tr><td></td><td></td><td></td></tr><tr><td></td><td></td><td></td></tr>';
            $('.group1 .half-l>tbody').html(group1_left);
            $('.group1 .half-r>tbody').html(group1_right);
            $('.group2 .half-l>tbody').html(group2_left);
            $('.group2 .half-r>tbody').html(group2_right);
        },
        ...
        groupFinish : function(json){
            ...
            if(j_len > 0){
                ...
                $.each(json.data,function(idx,val){
                    var this_data = this,
                        week_contract = COMM.formatNumber(this_data.week_contract,0),
                        week = COMM.formatNumber(this_data.week_total,0),
                        month_contract = COMM.formatNumber(this_data.month_contract,0),
                        month = COMM.formatNumber(this_data.month_total,0);
                    str += '<tr class="">'+
                ...
                swiper_3 = new Swiper('.swiper4', {
                    autoplay: 15000,
                    loop: true,
                    autoplayDisableOnInteraction: false
                });
                if(j_len <= 5){
                    swiper_3.stopAutoplay();
                }
            }
        },
        personalRate : function(json){
            ...
            if(j_len > 0){
                var htm = '', str = '', tab_len = 0;
                $.each(json.data,function(idx,val){
                    var this_data = this, month_val = COMM.formatNumber(this_data.month_mission,0), finish_val = COMM.formatNumber(this_data.month_finish,0);
                    //var rate = this_data.month_finish / this_data.month_mission
                    if(this_data.rate >= 100){
                        str += '<tr class="fc-red">';
                    }else if(this_data.rate > 0 && this_data.rate <= 50){
                        str += '<tr class="fc-green">';
                    }else if(this_data.rate <= 0){
                ...
                swiper_2 = new Swiper('.swiper3', {
                    autoplay: 15000,
                    loop: true,
                    autoplayDisableOnInteraction: false
                });
                if(j_len <= 5){
                    swiper_2.stopAutoplay();
                }
            }
        },
        groupRate : function(json){
            ...
            if(j_len > 0){
                var htm = '', str = '', tab_len = 0;
                $.each(json.data,function(idx,val){
                    var this_data = this, mission = COMM.formatNumber(json.data[idx].month_mission,0), finish = COMM.formatNumber(this_data.month_finish,0);
                    //console.log(mission)
                    //var rate = this_data.month_finish / this_data.month_mission
                    if(this_data.rate >= 100){
                        str += '<tr class="fc-red">';
                ...
                swiper_1 = new Swiper('.swiper2', {
                    autoplay: 15000,
                    loop: true,
                    autoplayDisableOnInteraction: false
                });
                if(j_len <= 5){
                    swiper_1.stopAutoplay();
                }
            }
        },
        ...
        swiperRank : function (){
            ...
            var swiper1 = new Swiper('#rank', {
                ...
                onSlideChangeEnd:function(e){
                    var eq_idx = $('.time-scale>li.active').index();

                    if(e.activeIndex === 1 || eq_idx >= 2){
                    ...
                    }else if(eq_idx === 0 && e.activeIndex !== 1) {
